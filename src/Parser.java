...
public class Parser {
    ...
    public Parser(ArrayList<Token> tokenList) {
        ...
        this.whileIndex = 0;
        this.analyze();
        this.recrusiveOutputTree(CompUnit);
    ...
    private int searchTable(Node node) {
        for (int i = globalSymbolTable.size() - 1; i >= 0; i--) {
            for (Node gnd : globalSymbolTable.get(i)) {
                if (gnd.getName() != null && node.getName() != null && gnd.getName().equals(node.getName())) {
...
//        Node CompUnit = new Node("CompUnit");
    ...
    public void Stmt(Node node) {
        ...
        if (curToken.type == SymbolType.IFTK) {
            ...
            ifToken.setNodeName("if");
            ifToken.setApearIndex(ifIndex);
            ifIndex++;
            insertTable(ifToken);
            ...
            if (curToken.type == SymbolType.LPARENT) {
                ...
                while (curToken.type == SymbolType.ELSETK) {
                    Node elseTk = getToken(Stmt);
                    elseTk.setNodeName("else");
                    elseTk.setApearIndex(elseIndex);
                    elseIndex++;
            ...
            whileToken.setNodeName("while");
            whileToken.setApearIndex(whileIndex);
            whileIndex++;
...
//            addTable();
            ...
            Node nodePtr = getToken(Stmt);
            nodePtr.setNodeName("continue");
            if (tbPtr - 2 <= 0) {
                error.addErrorMessage(nodePtr.getToken().getLine(), 'm');
            } else {
                boolean hasWhile = false;
                for (int i = tbPtr - 2; i >= 0; i--) {
                    // TODO dont know right or wrong
                    if (globalSymbolTable.get(i).size() != 0 && globalSymbolTable.get(i).get(globalSymbolTable.get(i).size() - 1).getName().equals("while")) {
                        hasWhile = true;
                        break;
                    }
                }
            ...
            for (int i = tbPtr - 2; i >= 0; i--) {
                // TODO dont knwo right or wrong
                if (globalSymbolTable.get(i).size() != 0 && (globalSymbolTable.get(i).get(globalSymbolTable.get(i).size() - 1).getType() == NodeInfo.INTFUN
                    || globalSymbolTable.get(i).get(globalSymbolTable.get(i).size() - 1).getType() == NodeInfo.VOIDFUN)) {
                    if (globalSymbolTable.get(i).size() != 0 && globalSymbolTable.get(i).get(globalSymbolTable.get(i).size() - 1).getType() == NodeInfo.VOIDFUN && hasValue) {
    ...
    public void EqExp(Node node) {
        ...
        while (curToken.type == SymbolType.EQL || curToken.type == SymbolType.NEQ) {
            if (curToken.type == SymbolType.EQL) {
                getToken(EqExp).setNodeName("==");
            } else {
                getToken(EqExp).setNodeName("!=");
            }
            RelExp(EqExp);
    ...
    public void RelExp(Node node) {
        ...
        while (curToken.type == SymbolType.LSS || curToken.type == SymbolType.LEQ || curToken.type == SymbolType.GRE || curToken.type == SymbolType.GEQ) {
            if (curToken.type == SymbolType.LSS) {
                getToken(RelExp).setNodeName("<");
            }
            else if (curToken.type == SymbolType.LEQ) {
                getToken(RelExp).setNodeName("<=");
            }
            else if (curToken.type == SymbolType.GRE) {
                getToken(RelExp).setNodeName(">");
            }
            else {
                getToken(RelExp).setNodeName(">=");
    ...
    public void ConstInitVal(Node node) {
        ...
        if (curToken.type == SymbolType.LBRACE) {
            getToken(ConstInitVal).setNodeName("{");
...
//        System.out.println(Number.getFinalValue());
    ...
    public Node LVal(Node node, Node funcName) {
        ...
        while (curToken.type == SymbolType.LBRACK) {
            getToken(LVal).setNodeName("[");
            av.putValueDimIndex("dim" + (dimension+1), Exp(LVal, garbage));
            if (curToken.type == SymbolType.RBRACK) {
                getToken(LVal).setNodeName("]");
            } else {
                Node tNode = new Node(new Token("]", SymbolType.RBRACK, idenfr.getToken().getLine()));
...
//            node.setFinalValue(name + "$" + idenfr.getApearIndex());
            if (dimension == 0) {
                ...
                LVal.setFinalValue(name + "$" + idenfr.getApearIndex() + "$arr");
                node.setFinalValue(name + "$" + idenfr.getApearIndex() + "$arr");
            }
        } else {
            av.setValue(name + "$" + idenfr.getApearIndex() + "$arr");
            av.setTarget("t" + "$$" + valueIndex);
            LVal.addArithExp(av);
            // TODO which ?
            //node.addArithExp(av);
            LVal.setFinalValue("t" + "$$" + valueIndex);
            node.setFinalValue("t" + "$$" + valueIndex);
            valueIndex++;
        }
    ...
    public LinkedHashMap<String, String> FuncFParam(Node node, Node funcName) {
        ...
        if (curToken.type == SymbolType.INTTK) {
            ...
            if (curToken.type == SymbolType.IDENFR) {
                ...
                if (curToken.type == SymbolType.LBRACK) {
                    dimension++;
                    getToken(FuncFParam).setNodeName("[");
                    if (curToken.type == SymbolType.RBRACK) {
                        getToken(FuncFParam).setNodeName("]");
                        while (curToken.type == SymbolType.LBRACK) {
                            dimension++;
                            getToken(FuncFParam).setNodeName("[");
                            // TODO 获取形参二维数组第二维度的size
                            ConstExp(FuncFParam);
                            if (curToken.type == SymbolType.RBRACK) {
                                getToken(FuncFParam).setNodeName("]");
                            } else {
                ...
                if (checkTable(idenfr)) {
                    ...
                    // TODO 数组中形式参数把标签开到10000+，便于判断
                    if (dimension != 0) idenfr.setApearIndex(paraIndex);
                    paraIndex++;
                } else {
                    error.addErrorMessage(idenfr.getToken().getLine(), 'b');
                }
                if (dimension == 0) {
                    ...
                    map.put(idenfr.getName() + "$" + idenfr.getApearIndex() + "$arr", "int");
                }
            } else {
                error();
            }
